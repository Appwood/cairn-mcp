"""Cairn GitHub social-preview card — 1280x640, 'Luminous Sediment'."""
...
def font(n, s): return ImageFont.truetype(f"{FONTS}/{n}", int(s))
...
def paint_stack(base, cx, bottom_y, unit):
    sh, gap = unit * 0.92, unit * 0.30; cells = []; y = bottom_y
    for wm in WIDTHS:
        w = unit * wm; cells.append((cx, y - sh / 2, w, sh)); y -= (sh + gap)
    top = cells[-1]; ecx, ecy = top[0], top[1] - top[3] * 0.5 - unit * 0.36
    ...
    for i, (scx, scy, w, h) in enumerate(cells):
        x0, y0 = scx - w / 2, scy - h / 2; is_top = (i == len(cells) - 1)
        ctop, cbot = (EMBER_TOP, EMBER_BOT) if is_top else (STONE_TOP, STONE_BOT); r = h * 0.4
        base.paste(vgrad(int(w), int(h), ctop, cbot), (int(x0), int(y0)), rmask(int(w), int(h), int(r)))
        ...
        dr = unit * 0.12; dcx, dcy = x0 + h * 0.5, scy
        d.ellipse([dcx - dr, dcy - dr, dcx + dr, dcy + dr], fill=DOTS[i % 4] + (235,))
        lx0 = dcx + dr * 1.8; lx1 = x0 + w - h * 0.4
        if lx1 > lx0:
...
ImageDraw.Draw(lift).ellipse([W * 0.03, -H * 0.3, W * 0.46, H * 1.3], fill=(13, 17, 26))
sheet = screen(sheet, lift.filter(ImageFilter.GaussianBlur(W * 0.08)))

unit = 84 * SS
paint_stack(sheet, 362 * SS, 470 * SS, unit)
...
x = tx; centers = []
for c in "cairn":
    w = d.textlength(c, font=f_word); d.text((x, word_y), c, font=f_word, fill=INK, anchor="lm")
    centers.append((x + w / 2, w)); x += w + 10 * SS
i_cx = centers[2][0]
tit = Image.new("RGB", sheet.size, (0, 0, 0))
tr = 16 * SS; tit_y = word_y - 90 * SS
ImageDraw.Draw(tit).ellipse([i_cx - tr, tit_y - tr, i_cx + tr, tit_y + tr], fill=GLOW)
sheet = screen(sheet, tit.filter(ImageFilter.GaussianBlur(20 * SS)))
d = ImageDraw.Draw(sheet, "RGBA")
...
def spaced(xy, text, fnt, fill, tracking):
    x = xy[0]
    for c in text:
        d.text((x, xy[1]), c, font=fnt, fill=fill, anchor="lm"); x += d.textlength(c, font=fnt) + tracking

spaced((tx + 5 * SS, 390 * SS), "evidence, not instructions", font("JetBrainsMono-Regular.ttf", 28 * SS), (150, 160, 178), 5 * SS)
spaced((tx + 5 * SS, 440 * SS), "structured log adapter · mcp", font("JetBrainsMono-Regular.ttf", 23 * SS), FAINT, 4 * SS)

sheet.resize((W // SS, H // SS), Image.LANCZOS).save(HERE / "cairn-social.png")
print("social card 1280x640 written")
