# Agent Instructions For Structured Logs
...
Use `search_logs` for discovery, `get_correlation_logs` for timelines,
`get_log_detail` for one selected traceback, and `get_log_stats` for scope.
